...
const LockyWidget = {
	...
	init() {
		...
		if (typeof lockyWidgetData === "undefined" || !lockyWidgetData.root_url)
			return;
	...
	cacheElements() {
		this.elements = {
			...
			confirmCancelBtn: document.getElementById("lk-confirmCancelBtn"),
			cancelResIdInput: document.getElementById("lk-cancel-res-id"),
			cancelCodeInput: document.getElementById("lk-cancelCode"),
	...
	initEventListeners() {
		...
		this.elements.lockForm.addEventListener("submit", (e) =>
		...
		if (this.elements.closeCancelModal) {
			this.elements.closeCancelModal.addEventListener("click", () =>
				this.closeCancelModal(),
			);
		}
		this.elements.cancelForm.addEventListener("submit", (e) =>
			this.handleCancelSubmit(e),
		);
	...
	loadWidgetData() {
		...
		Promise.all([
			...
			.catch((err) => {
				...
				this.elements.initialLoading.textContent =
		...
		fetch(`${this.baseUrl}generate-code`, {
			...
			.then((data) => {
                ...
                `;
					...
					fetch(`${this.baseUrl}get-all-reservations`)
						.then((res) => res.json())
	...
	setSubmitCancelState(isLoading) {
		...
		this.elements.cancelForm.style.cursor = isLoading
			? "not-allowed"
			: "default";
		this.elements.confirmCancelBtn.disabled = isLoading;
		this.elements.confirmCancelBtn.textContent = isLoading
			? "Annulation..."
			: "Confirmer l'annulation";
	},

	resetCancelResultBox() {
		this.elements.cancelResultBox.style.display = "none";
		this.elements.cancelResultBox.textContent = "";
		this.elements.cancelResultBox.classList.remove(
			"is-neutral",
			"is-success",
			"is-error",
		);
	...
	setCancelResultState(status, message) {
		this.elements.cancelResultBox.classList.remove(
			"is-neutral",
			"is-success",
			"is-error",
		);
		this.elements.cancelResultBox.classList.add(`is-${status}`);
	...
	initFullCalendar(reservations) {
		if (!this.elements.calendarEl || typeof FullCalendar === "undefined")
			return;
		...
		const events = reservations.map((res) => {
			...
			return {
				...
				title:
					res.client_name + (res.client_phone ? ` (${res.client_phone})` : ""),
	...
	renderLegend() {
		...
		Object.keys(this.lockColorMap).forEach((lockId) => {
			...
			label.textContent =
				this.lockNames[lockId] || `Cadenas #${lockId.substr(-4)}`;
	...
	handleCancelSubmit(e) {
		...
		const code = this.elements.cancelCodeInput.value;

		this.setCancelResultState("neutral", "Vérification...");
		...
		fetch(`${this.baseUrl}cancel-reservation`, {
			...
			.then((data) => {
				if (data.success) {
					this.setCancelResultState(
						"success",
						"Réservation annulée ! Le calendrier se met à jour...",
					);
